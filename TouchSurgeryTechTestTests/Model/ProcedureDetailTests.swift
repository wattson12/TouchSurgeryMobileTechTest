...
class ProcedureDetailTests: XCTestCase {

    func testMappingFromJSON() {
        do {
            let detail = try JSONDecoder().decode(ProcedureDetail.self, from: testData(fromFixtureNamed: "procedure_detail"))
    ...
    func testCreatingEmptyVersionFromProcedure() {
        ...
        let detail = ProcedureDetail(emptyDetailFromProcedure: procedure)
